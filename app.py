...
st.caption("Built for tracking/enriched event data (xThreat/xShot/xLoss + positions). Export PDFs from current filters.")
...
with st.sidebar:
    ...
    # Match selection (always available)
    ...
    # Date range (only if match_date attached)
    if has_dates:
        ...
        # Fallback: match_id bounds (acts like a 'range')
        mmin, mmax = available_match_bounds(df)
        mrange = st.slider("Match ID range (fallback)", min_value=int(mmin), max_value=int(mmax), value=(int(mmin), int(mmax)))
        date_start, date_end = None, None  # handled in apply_filters

    # Phase / flags
    phase_opts = sorted(df["team_in_possession_phase_type"].dropna().unique().tolist())
...
with colB:
    if has_dates and state.date_start and state.date_end:
        st.caption("PDF is generated from the current filters (including date range).")
    else:
        st.caption("PDF is generated from the current filters. Upload match metadata to enable date ranges.")
